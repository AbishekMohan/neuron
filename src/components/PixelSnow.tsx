...
// Adapted from React Bits' PixelSnow (JS + CSS variant), typed for this
// project, with deliberately conservative defaults: this runs its own
// separate WebGLRenderer (distinct from the r3f Canvas used elsewhere),
// with a 128-iteration raymarched shader per pixel. Defaults here trade
// some visual density for real cost: farPlane cut from 20 to 10 (roughly
// halves the average loop iterations before the ray exits), pixel ratio
// capped at 1 instead of up to 2 (a real DPI display was the single
// biggest lever we found for this exact page's lag earlier), and
// pixelResolution/density lowered. Tune upward only after confirming the
// page still feels smooth.
...
export default function PixelSnow({
  ...
  return (
    <div
      ref={containerRef}
      className={className}
      style={{ width: '100%', height: '100%', position: 'relative', overflow: 'hidden', contain: 'layout style paint', ...style }}
    />
  );
}
